...
#		see(\%record);
	...
	if($id_code_cible == 7)
	{
		...
		$record{facture_nom} = uc($record{facture_nom});
...
my $ID_Table_commission = $check_comm{id}|0;

	#ajout commission pour cette commande
	log_debug("if($ID_Table_commission == 0 && $member_id_commission > 0 && $montant_commission > 0)",'','save_doc_facture_commission');
	#if(!($check_comm{id} > 0) && $member_id_commission > 0 && $montant_commission > 0) {
	if( $ID_Table_commission > 0 && $member_id_commission > 0 && $montant_commission > 0) {
...
# 	if(($ID_Table_commission eq '0') && $member_id_commission > 0 && $montant_commission > 0) {
...
# 				commande_id  => $facture{id},
# 				montant => $montant_commission,
# 				date_commission => 'CURDATE()'
# 			);

# 			my $newCommission = sql_set_data({ dbh => $dbh, debug => 1, table => 'handmade_certigreen_commissions', data => \%update_commission });
...
sub get_html_facture
{
	...
	($head,$body,$number) = add_page({type=>'footer',number=>$number, prefixe=>$prefixe,id=>$id,facture=>\%facture, license=>\%license,head=>$head,body=>$body});
...
sub add_page
{
	...
	my %member_commission = %{$d{member_commission}};
...
EOH
	...
	{
		$d{head} .= add_bloc_head({size=>'958',number=>$d{number}});
	...
	elsif($d{type} eq 'commission')
	{
		$d{head} .= add_bloc_head({size=>'958',number=>$d{number}});
		$nouvelle_page = add_page_content({bloc_page=>$bloc_page,content=>$d{content},bloc_client=>$bloc_client_document,license=>\%license,commande_document=>\%commande_document,facture=>\%facture,number=>$d{number}++,prefixe=>$d{prefixe},pj_name=>$pj_name, member_commission=>\%member_commission});
	}
...
EOH
		...
		<div class="footer_certigreen" style="position:absolute;bottom:10px;">
...
#	if (@document_details > 3) {
#		#Ajoute l'image dans une nouvelle page
#
#		$page_content .= <<"EOH";
#        <div style="page-break-before: always;"></div>
#		 <div class="footer_certigreen2" style="position:absolute;top:150px;">
#	         <a href="https://www.certigreen.be/public/connexion"><img style="width:640px; height:88px; margin-left:80px; display:block;" src='https://www.certigreen.be/skin/img/footer_document.png'/></a>
#	         <br/>
#	     </div>
#EOH
#
#	} else {
#	$page_content .= <<"EOH";
#		<div class="footer_certigreen2" style="position:absolute;bottom:280px;">
#	<a href="https://www.certigreen.be/public/connexion"><img style="width:640px; height:88px; margin-left:80px; display:block;" src='https://www.certigreen.be/skin/img/footer_document.png'/></a>
#	<br/>
#	</div>
#EOH
...
#	<br>
#        <div style="page-break-before: always;"></div>
#		<div class="footer_certigreen2" style="position:absolute;top:200px;">
#	<a href="https://www.certigreen.be/public/connexion"><img style="width:640px; height:88px; margin-left:80px; display:block;" src='https://www.certigreen.be/skin/img/footer_document.png'/></a>
#	<br/>
#	</div>
#EOH
...
#		see(\%member);
	...
	if($member_commission{id})
	{
		#coordonnées client
		$d{document} =~ s/\-\-\-client_name\-\-\-/$member_commission{firstname} $member_commission{lastname}/g;
		$d{document} =~ s/\-\-\-client_adres1\-\-\-/$member_commission{street} $member_commission{number}/g;
		$d{document} =~ s/\-\-\-client_adres2\-\-\-/$member_commission{zip} $member_commission{city}/g;
		$d{document} =~ s/\-\-\-client_adres3\-\-\-/Belgique/g;
		$d{document} =~ s/\-\-\-client_vat\-\-\-//g;
		$d{document} =~ s/\-\-\-client_email\-\-\-/$member_commission{email}/g;
	}
...
sub add_bloc_head
{
	...
	return <<"EOH";
		if(jQuery('#pagescontent$number').contents().length > 0)
		{
			...
			jQuerypage.find(".footer").append("<p style='text-align:right'>"+page+"</p>");
...
sub save_doc_commission_cree_pdf
{
	my %commission = %{$_[0]};
	my $member_id = $commission{member_id};
	...
	if($commission{id} > 0) {
		...
		my $filename = get_document_name({ date => 0, prefixe => 'co', id => $member_id, type => 'document' });

		#my $url = 'https://certigreen.dev.bugiweb.net/cgi-bin/adm_handmade_certigreen_commandes.pl?&sw=get_html_document_commission&id='.$commission{id}.'';
		my $url = 'https://certigreen.be/cgi-bin/adm_handmade_certigreen_commandes.pl?&sw=get_html_document_commission&id='.$commission{id}.'';


		my $dir = $config{directory_path} . '/usr/files/CO';
		my $path_temp = $config{directory_path} . '/usr/documents/' . $filename . '_temp.pdf';
		my $path_new = $config{directory_path} . '/usr/documents/' . $filename . '.pdf';

		log_debug("PDF Commission: ".$path_new);
...
sub get_html_document_commission
{
	my $commission_id = get_quoted('id');


	my %commission = sql_line({debug=>0,debug_results=>0,table=>'handmade_certigreen_commissions',where=>"id='$commission_id'"});
	my $member_id = $commission{member_id};


	my @commissions = sql_lines({debug=>0,debug_results=>0,table=>'handmade_certigreen_commissions',where=>"member_id='$member_id'"});
	my %member = sql_line({debug=>0,debug_results=>0,table=>'members',where=>"id='$member_id'"});
...
EOH
	 ...
	 foreach $commission (@commissions) {
		 ...
		 if ($commission{id} > 0) {
			 ...
			 %intranet_facture = sql_line({ debug => 0, debug_results => 0, table => 'intranet_factures', where => "id ='$commande_id'" });

			 $content .= <<"EOH";
		<tr>
			<td>
		 $intranet_facture{lastname}  $intranet_facture{firstname}
		</td>
			<td>
		 $intranet_facture{street}  $intranet_facture{number}<br />
		$intranet_facture{zip} $intranet_facture{city}
		</td>
			<td>
EOH

				 my $factureOk = 0;
				 my $statutFacture = 'Attente paiement';
				 my $styleFacture = 'background-color: #f28410; color:white;';

				 if ($intranet_facture{statut} == 4 || $intranet_facture{forcer_paye} == 'y') {
					 $factureOk = 1;
					 $statutFacture = 'Payée';
					 $styleFacture = 'background-color: #80c53a; color:white;'
				 }

				 $content .= <<"EOH";
		          <div style="$styleFacture padding:5px;text-align:center;" >

		           FA$intranet_facture{numero_facture}
			      </div>
			</td>
            ...
            <td style="color:red">
EOH
			    if ($factureOk == 0 && $commission{paye} ne 'y') {
				 $content .= $intranet_facture{montant_commission} . '€ TVAC';
			    }
			     $content .= <<"EOH";
             </td>
...
EOH
			    ...
			    if ($factureOk == 1 && $commission{paye} ne 'y') {
				 $content .= $intranet_facture{montant_commission} . '€ TVAC';
			    }
...
#	$content = def_handmade::map_document($content,'commande_documents',$id,'do');
	...
	($head,$body,$number) = add_page({type=>'commission',footer=>0,content=>$content,number=>$number, prefixe=>$prefixe,id=>$id,commande_document=>\%commande_document, license=>\%license,head=>$head,body=>$body, member_commission=>\%member});

#	#habillage document

	my $html_document = get_html_document_set_canvas({head=>$head,body=>$body,license=>\%license});
