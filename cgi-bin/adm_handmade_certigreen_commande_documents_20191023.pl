...
sub breadcrumb_func
{
	...
	<ol class="breadcrumb">
			...
			<li><a href="/admin-certigreen/intranet_factures&sel=1000278">Commandes</a></li>
...
sub action_globale_prevenir_client
{
	...
	foreach $id (@ids)
	{
		if($id > 0)
		{
			...
			if(!($member{id_member_agence} > 0))
			{
				$destinataire = $commande{email};
				$dest_nom = $commande{firstname}." ".$commande{lastname};
			}
			elsif (($member{type_member} eq "Particulier") && ($member{id_member_agence} != 0 ))
			{
				my %agence = sql_line({dbh=>$dbh2,debug=>0,debug_results=>0,table=>'members',where=>"id='$member{id_member_agence}'"});
