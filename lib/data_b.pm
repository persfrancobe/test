...
sub compute_sheets
{
	...
	my %nb_total_resultats = sql_line({select=>"COUNT(*) as nb",table=>$table,where => $where,debug => $debug,debug_results => $debug});
	...
	my $i=0;
	# my $url_detail = "where:[$where]limit:[$limit]ordby:[$ordby]".get_data_detail_url($dbh,\%data_sheet,$colg,$extlink,'y');   
	my $url_detail = get_data_detail_url($dbh,\%data_sheet,$colg,$extlink,'y');   
		
	my @data_sheets = sql_lines({debug => 1,debug_results => $debug,select=>$select, table=>$table,where => $where,ordby => $ordby,limit => $limit});
...
sub get_pagination_old
{
	...
	my $pagination=<<"EOH";
            <table id="pagination_tab">
                <tr>
					<td class=""> <a class="pagination_previous $prev_disabled"   href="$link_first">
						<span>&laquo;</span>
				   </a> 
				   </td>
					
					<td class=""><a class="pagination_next $prev_disabled"  href="$link">
						<span>&lsaquo;</span>
				   </a> 
				   </td>
...
EOH
    if($end_page != 1)
    {
        for $number_page ($page_interval_1 .. $page_interval_2)
        {
            ...
            my $page_class="pagination_page";
            my $page_link='<a href="'.$link.'"> '.$number_page.' </a>';
            if($number_page == $d{current_page})
            {
                $page_class="pagination_page_selected";
                $page_link='<span>'.$number_page.'</span>';
            }
...
EOH
    ...
    if($next_page <= $page_interval_2)
    {
        $next_link=<<"EOH";
          <td class="">
		  <a class="pagination_next" href="$link">
                <span>&rsaquo;</span>
           </a>
		   </td>
		   <td class="">
		   <a class="pagination_last" href="$link_last">
                <span>&raquo;</span>
           </a>
		   </td>
EOH
    ...
    $pagination.=<<"EOH";
                    ...
                    <td class="nbr_results">
                        <span>
                            $d{nb_total_resultats}
                        </span> 
                        $sitetxt{products_mp_results}
                    </td>
                </tr>
            </table>
EOH
...
sub get_data_url
{
	...
	# log_debug("$sf:$lg: $url .= $rew_names_sfs{$lg}{$sf}.'/';",'','debug_sitemap');
...
sub map_list_listing
{
	  ...
	  my $gotobox = get_gotobox($last_page);
...
sub get_tri
{
	...
	my $link_az = data::get_data_url({sf=>$sf,nr=>$nr,from=>'get_tri',params=>'tri=2'});
	my $link_prix_croissant = data::get_data_url({sf=>$sf,nr=>$nr,from=>'get_tri',params=>'tri=3'});
	my $link_prix_decroissant = data::get_data_url({sf=>$sf,nr=>$nr,from=>'get_tri',params=>'tri=4'});
...
EOH
	...
	return <<"EOH";
		<form method="post" action="$self" class="form-inline" role="form">
			...
			<strong>Aller page</strong> <input type="text" name="page" class="form-control" style="width:40px;" value="" />  / $end_page	
...
sub data_get_html_object
{
  ...
  my %eshop_setup = sql_line({debug=>0,table=>'eshop_setup'});
  ...
  {
	  %lowest_data_stock_tarif = sql_line({debug=>0,debug_results=>0,table=>"data_stock_tarif dst, data_stock ds",select=>"",ordby=>"st_pu_tvac asc",limit=>'0,1',where=>"st_pu_tvac >0 AND dst.id_data_stock = ds.id AND dst.id_tarif = '$tarif{id}' AND dst.id_data_sheet = '$sheet{id}' $where_stock"});
  ...
  for ($i_html = 0; $i_html<$#balises+1; $i_html++ ) 
  {
      ...
      my ($type,$element,$precision)= split(/\_/,$balise);
...
sub data_get_html_object_pic
{
    ...
    if($element eq 'ALL')
    {
         my @pics = sql_lines({debug=>0,debug_results=>0,table=>"migcms_linked_files lnk",where=>"lnk.table_name='data_sheets' AND table_field = 'photos' AND lnk.visible='y'  AND lnk.token = '$sheet{id}'",ordby=>'ordby'});
         foreach $pic(@pics)
         {
             my %pic=%{$pic};
    ...
    elsif($element > 0)
    {
          $element--;
          my $limit =  $element.', 1 ';
		  my %pic = sql_line({debug=>0,debug_results=>0,table=>"migcms_linked_files lnk",where=>"lnk.table_name='data_sheets' AND table_field = 'photos' AND lnk.visible='y' AND lnk.token = '$sheet{id}'",ordby=>'ordby',limit=>$limit});
...
EOH
	...
	foreach $data_stock(@data_stocks)
	{
		...
		$table .= <<"EOH";
			<tr>
				<td>
					<input type="checkbox" value="$data_stock{id}" checked="checked" name="cb_id_data_stock_$data_stock{id}" id="$data_stock{id}" class="cb_id_data_stock" />
				</td>
				<td><span class="category_fahter">$category_name_father: </span><span class="category_name">$category_name</span></td>
				<td class="data_stock_stock">$data_stock{stock}</td>
				<td class="data_stock_weight">$data_stock{weight} Kg</td>
				<td class="data_stock_price" abbr="$sheet_prices{price_tvac}"></td>
				<td class="data_stock_price_discount" data-discounted="$sheet_prices{discounted}" data-discounttaux="$sheet_prices{discount_taux}" abbr="$sheet_prices{price_discounted_tvac}"></td>
...
sub data_get_html_object_input
{
    ...
    my $input=<<"EOH";
    <input type="text" class="data_quantity_to_add_$sheet{id} form-control required" id="data_quantity_to_add_$sheet{id}" value="1" />
EOH
...
sub recurse_categories_menu
{
	...
	my @categories = sql_lines({debug=>0,debug_results=>0,table=>'data_categories',where=>"visible='y' AND id_father='$d{id_father_category}' AND id_data_family='$data_family{id}' ",ordby=>"ordby"});
	foreach $categorie_ref (@categories)
	{
		my %categorie=%{$categorie_ref};
		my $title = get_traduction({debug=>0,id_language=>$config{current_language},id=>$categorie{id_textid_name}});
		my $nb = '';
		if($sf{count_sheets} eq 'y')
		{
			my ($list_sheets,$nb_total_resultats) = compute_sheets({nb_total_seulement=>'y',force_id_data_category=>$categorie{id},data_setup=>\%data_setup,data_family=>\%data_family,tarif=>\%tarif,current_page=>1,tri=>$tri,nr=>$data_family{family_nr},lg=>$config{current_language}});
			$nb = $nb_total_resultats;
		}		
		my $link = $config{baseurl}."/".data::get_data_url({lg=>$config{current_language},params=>'',reset=>$d{reset},sf=>$sf{id},number_page=>1,id_father_categorie=>$categorie{id},nr=>$data_family{family_nr},from=>'recurse_categories_menu', force_s1_value=>$force_s1_value});
		$link =~ s/\/\//\//g;
