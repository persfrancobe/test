...
sub member_events_to_mailing_tags
{
	...
	#traite automatiquement les events qui ne sont pas utiles au mailing
	execstmt($dbh,"update `migcms_members_events` SET trans = 1 where trans = 0 AND type_evt NOT IN ('queue_mailing','sent_mailing','open_mailing','click_mailing')");
	...
	if($id_dir_envoi > 0 && $id_dir_lecture > 0 && $id_dir_clic > 0 && $config{tag_from_date} ne '')
	{
		...
		# {
			...
			my @migcms_member_events = sql_lines({select=>"id,id_member,detail_evt,type_evt",table=>"migcms_members_events",ordby=>"id desc",where=>"trans = 0",limit=>"$limit"});
...
sub send_mailings
{
	...
	foreach $mailing_sending (@mailing_sendings)
	{
		...
		if($mailing_sending{id} > 0)
		{
			...
			my $where = mailing::mailing_get_where_member({id_member_rule=>$mailing_sending{id_member_rule},variable_a=>$mailing_sending{variable_a},variable_b=>$mailing_sending{variable_b},tags=>$mailing_sending{tags},groupes=>$mailing_sending{groupes},tag_interdit=>$mailing_sending{tag_interdit},emails_test=>$mailing_sending{emails_test},num_optout=>$num_optout,click_depuis=>$mailing_sending{click_depuis},ouvert_depuis=>$mailing_sending{ouvert_depuis},ajout_pas_recu_cette_nl=>$mailing_sending{ajout_pas_recu_cette_nl},ajout_pas_recu_aucune_nl=>$mailing_sending{ajout_pas_recu_aucune_nl},id_migcms_page=>$mailing_sending{id_migcms_page}});
			...
			foreach $email (@emails)
			{
				...
				{			
					...
					if($config{mqd_data2_func} ne "")
					{
						my $mqd_data2_func = 'def_handmade::'.$config{mqd_data2_func};
						$f2 = &$mqd_data2_func({member=>\%email});
					}
