...
$dm_cfg{'list_custom_action_2_func'} = \&voir_sheets;
$dm_cfg{'list_custom_action_3_func'} = \&sort_sheets;
...
my $id_father_datatype = "autocomplete";
if($config{data_categories_light_id_father_datatype} ne "")
{
	$id_father_datatype = $config{data_categories_light_id_father_datatype};
}
...
$dm_cfg{self} = "$config{baseurl}/cgi-bin/adm_$dm_cfg{table_name}_light.pl?id_data_family=$id_data_family&colg=$colg&id_father=$id_father";
...
%dm_dfl = (
      ...
      # '10/block_robots'=> 
       # {
	        # 'title'=>'Empêcher le référencement des enfants',
	        # 'fieldtype'=>'checkbox',
			# tab=>3,
	    # }
       ,
...
sub enfants
{
	...
	my $colg = $_[1];
	my %record = %{$_[2]};
	my $self = "$config{baseurl}/cgi-bin/adm_$dm_cfg{table_name}_light.pl?id_data_family=$id_data_family&colg=$colg&id_father=$id&sel=$sel";
	
	my $acces = <<"EOH";
		<a class="btn btn-primary" href="$self" data-original-title="Voir les catégories enfant" target="" data-placement="bottom">
