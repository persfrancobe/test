...
(
      ...
      # }
      ,
      '23/url_info_livraison'=> 
      {
      'title'=>"Page 'Frais de port'",
	  'tab'=>'panier',
      'fieldtype'=>'listboxtable',
      'lbtable'=>'pages, textids, textcontents',
      'lbkey'=>'pages.id',
      'lbdisplay'=>'textcontents.content',
      'lbwhere'=>"pages.id_textid_name = textids.id AND textids.id = textcontents.id_textid AND id_language = 1"
      }
      ,
 ...
 '74/member_autologin_after_signup'=>
      {
        'title'=>"Connex. auto après inscr. (membre)",
        'fieldtype'=>'checkbox',
		'tab'=>'member',
        'checkedval' => 'y',
      },
 '75/revendeur_autologin_after_signup'=>
      {
        'title'=>"Connex. auto après inscr. (revendeur)",
        'fieldtype'=>'checkbox',
		'tab'=>'member',
        'checkedval' => 'y',
      },
...
#     fill_eshop_logs();
    ...
    #CHECK FIELD LABELS VALIDITY intéret ?
    ...
    #####################################
    # GENERATION DES PDF DE RETRACTATION
    #####################################
    
    # On check si le multilangue est activée
    my %multilangue = sql_line({dbh=>$dbh, table=>"config", where=>"varname = 'multi_language'"});

    my %eshop_setup = sql_line({dbh=>$dbh, table=>"eshop_setup", where=>"id='$id_banner'"});    


    my @tab_langues;
    if($multilangue{varvalue} eq "y")
    {
      # Récupération des langues activées
      ...
      foreach $language (@languages)
      {
        ...
        push @tab_langues, \%language;
      }
    }
    else
    {
      # Sinon on ne récupère que le français
      $language{id} = 1;
      $language{name} = "fr";
      push @tab_langues, \%language;
    }
    ...
    foreach $tab_langue (@tab_langues)
    {
      %tab_langue = %{$tab_langue};

      # Si le fichier de modele existe
      $filename = "../mig_skin/modele_retraction_".$tab_langue{id}.".pdf";
      if (-e $filename) 
      {     
        # Récupération des urls
        my %url_retraction = sql_line({
          dbh=>$dbh,
          table=>"pages, textids, textcontents as text",
          where=>"pages.id = '$eshop_setup{id_page_retraction}' AND pages.id_textid_refurl = textids.id AND text.id_textid = textids.id AND text.id_language = '$tab_langue{id}'"
        });
        
        my $complete_url_retraction = "$config{baseurl}/" . $url_retraction{content};

        my %url_contact = sql_line({
          dbh=>$dbh,
          table=>"pages, textids, textcontents as text",
          where=>"pages.id = '$eshop_setup{id_page_contact}' AND pages.id_textid_refurl = textids.id AND text.id_textid = textids.id AND text.id_language = '$tab_langue{id}'"
        });
      
        my $complete_url_contact = "$config{baseurl}/" . $url_contact{content};
        

        my %infos = (
          filename       => $eshop_setup{eshop_name}." - retractation ".$tab_langue{name},
          modele_path    => "../mig_skin/modele_retraction_".$tab_langue{id}.".pdf",
          eshop_name     => $eshop_setup{eshop_name},
          delai          => $eshop_setup{delai_retraction},
          url_retraction => $complete_url_retraction,
          url_contact    => $complete_url_contact,
          email          => $eshop_setup{eshop_email},
          street         => $eshop_setup{eshop_street},
          zip_city       => $eshop_setup{eshop_zip_city},
          country        => $eshop_setup{eshop_country},
        );

        my $file = create_retraction_pdf(\%infos);


        open (FILE, '>', "$file_display");
        print FILE $file_display;
        close FILE;  	  }
    }
}
