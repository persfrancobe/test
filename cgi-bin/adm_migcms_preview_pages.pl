...
$dm_cfg{wherep} = "(migcms_pages_type = 'page' OR  migcms_pages_type = 'link' OR migcms_pages_type = 'directory')";
$dm_cfg{table_name} = "migcms_pages";
$dm_cfg{list_table_name} = "$dm_cfg{table_name}";
$dm_cfg{default_ordby} = "ordby";
$dm_cfg{self} = "$config{baseurl}/cgi-bin/adm_migcms_preview_pages.pl?";
...
sub page_preview
{
  ...
  if($page{migcms_pages_type} eq 'directory')
  {
	  ...
	  my $size1 = length($elt_name);
	  $elt_name = substr( $elt_name, 0, 15 );
	  my $size2 = length($elt_name);
	  if($size1 != $size2)
	  {
		$elt_name .= '...';
	  }
      ...
      $elt = <<"EOH";
<i class="fa fa-folder-open-o"></i> $elt_name
EOH
  }
  elsif($page{migcms_pages_type} eq 'link')
  {
      $elt = <<"EOH";
<i class="fa fa-link"></i><i> $elt_name</i>
EOH
  }
  elsif($page{migcms_pages_type} eq 'page')
  {
	  ...
	  my $sel_page = get_quoted('sel_page');
	  ...
	  $elt = <<"EOH";
		<a href="$config{baseurl}/cgi-bin/adm_migcms_parag.pl?id_page=$page{id}&sel=$sel_page" class="$class"><i class="fa fa-file-text-o"></i>
		&nbsp; $lock $elt_name &nbsp;&nbsp;
		</a>
EOH
  }
