...
sub render_parag
{
	...
	($text5,$dum) = get_textcontent($dbh,$parag{id_textid_text_5});
	...
	($textwysiwyg5,$dum) = get_textcontent($dbh,$parag{id_textid_textwysiwyg_5});
  ...
  $textwysiwyg5 =~ s/\&lt;/</g;	
	...
	$template{template} =~ s/<MIGC_PARAGTEXT5_HERE>/$text5/g; 
	...
	$template{template} =~ s/<MIGC_PARAGCONTENT5_HERE>/$textwysiwyg5/g; 
...
# print $d{preview};
   ...
   my @pagescontents = (/<MIGC_PAGECONTENT_\[(\w+)\]_HERE>/g);
	...
	for ($i = 0; $i<=$#blocks; $i++ ) 
	{
		# if ($blocks[$i] =~ /^T(\d+)/ && $d{type_page} ne '') 
		if ($blocks[$i] =~ /^T(\d+)/) 
		{
			my $id_blocktype = $1;
